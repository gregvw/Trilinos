...
private:
  const std::shared_ptr<FEM<Real> > FEM_;
  int num_solves_;
...
public:
  DiffusionConstraint(const std::shared_ptr<FEM<Real> > &FEM) : FEM_(FEM), num_solves_(0) {}
  ...
  void value(ROL::Vector<Real> &c, const ROL::Vector<Real> &u, const ROL::Vector<Real> &z, Real &tol) {
    ...
    std::shared_ptr<const std::vector<Real> > up = dynamic_cast<const ROL::StdVector<Real>&>(u).getVector();
    std::shared_ptr<const std::vector<Real> > zp = dynamic_cast<const ROL::StdVector<Real>&>(z).getVector();
    // Apply PDE Jacobian
  ...
  void solve(ROL::Vector<Real> &c, ROL::Vector<Real> &u, const ROL::Vector<Real> &z, Real &tol) {
    std::shared_ptr<std::vector<Real> > cp = dynamic_cast<ROL::StdVector<Real>&>(c).getVector();
    std::shared_ptr<std::vector<Real> > up = dynamic_cast<ROL::StdVector<Real>&>(u).getVector();
    std::shared_ptr<const std::vector<Real> > zp = dynamic_cast<const ROL::StdVector<Real>&>(z).getVector();
    // Get PDE Jacobian
...
public:
  DiffusionObjective(const std::shared_ptr<FEM<Real> > fem, const Real alpha = 1.e-4)
  ...
  void gradient_1( ROL::Vector<Real> &g, const ROL::Vector<Real> &u, const ROL::Vector<Real> &z, Real &tol ) {
    std::shared_ptr<std::vector<Real> > gp = dynamic_cast<ROL::StdVector<Real>&>(g).getVector();
    std::shared_ptr<const std::vector<Real> > up = dynamic_cast<const ROL::StdVector<Real>&>(u).getVector();
    int nu = FEM_->nu();
    // COMPUTE GRADIENT
    std::vector<Real> diff(nu);
  ...
  void hessVec_11( ROL::Vector<Real> &hv, const ROL::Vector<Real> &v,
             const ROL::Vector<Real> &u, const ROL::Vector<Real> &z, Real &tol ) {
    std::shared_ptr<std::vector<Real> > hvp = dynamic_cast<ROL::StdVector<Real>&>(hv).getVector();
    std::shared_ptr<const std::vector<Real> > vp = dynamic_cast<const ROL::StdVector<Real>&>(v).getVector();
    // COMPUTE HESSVEC
