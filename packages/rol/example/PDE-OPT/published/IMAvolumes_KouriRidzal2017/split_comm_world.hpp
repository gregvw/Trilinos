void split_comm_world(std::shared_ptr<Teuchos::Comm<int> > & Comm_linalg,
                      std::shared_ptr<Teuchos::Comm<int> > & Comm_sample,
                      int M) {
  ...
  Comm_linalg = std::make_shared<Teuchos::MpiComm<int>>(linalg_comm); // Wrap as Teuchos::MpiComm.
  ...
  Comm_sample = std::make_shared<Teuchos::MpiComm<int>>(sample_comm); // Wrap as Teuchos::MpiComm.
}
