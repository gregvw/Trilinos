...
public:
  PDE_OptVector_BatchManager(const std::shared_ptr<const Teuchos::Comm<GO> > &comm)
  ...
  void sumAll(ROL::Vector<Real> &input, ROL::Vector<Real> &output) {
    ...
    std::shared_ptr<ROL::TpetraMultiVector<Real,LO,GO,Node> > input_field_ptr
      = dynamic_cast<OptVector&>(input).getField();
    std::shared_ptr<ROL::TpetraMultiVector<Real,LO,GO,Node> > output_field_ptr
      = dynamic_cast<OptVector&>(output).getField();

    if ( input_field_ptr != nullptr ) {
      std::shared_ptr<FieldVector> input_field  = input_field_ptr->getVector();
      std::shared_ptr<FieldVector> output_field = output_field_ptr->getVector();
    ...
    std::shared_ptr<ROL::StdVector<Real> > input_param_ptr
      = dynamic_cast<OptVector&>(input).getParameter();
    std::shared_ptr<ROL::StdVector<Real> > output_param_ptr
      = dynamic_cast<OptVector&>(output).getParameter();

    if ( input_param_ptr != nullptr ) {
      std::shared_ptr<ParamVector> input_param  = input_param_ptr->getVector();
      std::shared_ptr<ParamVector> output_param = output_param_ptr->getVector();
