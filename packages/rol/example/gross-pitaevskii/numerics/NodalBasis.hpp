...
struct NodalBasis{
        ...
        NodalBasis(std::shared_ptr<Teuchos::LAPACK<int,Real> > lapack, const int ni, const int nq);    
        ~NodalBasis();
        ...
        std::shared_ptr<Lagrange<Real> > lagrange_; 
...
*/
template<class Real>
NodalBasis<Real>::NodalBasis(std::shared_ptr<Teuchos::LAPACK<int,Real> > const lapack, const int ni, const int nq): 
    ...
    lagrange_ = std::make_shared<Lagrange<Real>>(xi_,xq_);
